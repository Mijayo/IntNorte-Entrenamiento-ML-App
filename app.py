...
""")
    ...
    r2.markdown(kpi("Reducción error",  "~70%",          "📉", "amber"), unsafe_allow_html=True)
    ...
    ahorro_pesos_mes      = ahorro_unidades_mes * PRECIO_UNITARIO * (COSTO_CAPITAL / 12)

    e1, e2, e3 = st.columns(3)
    e1.markdown(kpi("Ahorro inventario/mes",
                    f"${ahorro_pesos_mes:,.0f}",   "💵", "green"), unsafe_allow_html=True)
    e2.markdown(kpi("Ahorro anual estimado",
                    f"${ahorro_pesos_mes*12:,.0f}", "📈", "amber"), unsafe_allow_html=True)
    e3.markdown(kpi("Unidades rescatadas/mes",
                    f"{ahorro_unidades_mes:.1f}",   "📦"),           unsafe_allow_html=True)

    st.markdown(f"""<div class="info-box">
<strong>Supuestos del cálculo:</strong> precio unitario Tiggo 2 = $320,000 MXN &nbsp;·&nbsp;
costo de capital = 15% anual &nbsp;·&nbsp; error baseline (criterio subjetivo) = 20% &nbsp;·&nbsp;
MAPE modelo = {MAPE:.1f}%. El ahorro representa la reducción en capital inmovilizado por unidades
sobrecompradas.
</div>""", unsafe_allow_html=True)
